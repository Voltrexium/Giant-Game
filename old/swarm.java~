...
public class swarm extends JFrame
{
    ...
    public void run() throws InterruptedException, IOException
    {
        ...
        {
            money2 = money2 - 100;
            swarm.main(input2,  money2, xy.size(), points);
        }
        inputForGame2.main(input2, money2, image);
    }
    ...
    protected void processInput() throws InterruptedException
    {
        ...
        {
            level++;
            int x2 = rand.nextInt( WIDTH - 100);
            int y2 = rand.nextInt( HEIGHT - 100);
            while (x2 < 0)
            {
                x2 = rand.nextInt( WIDTH - 100);
            }
            while (y2 < 0)
            {
                y2 = rand.nextInt( HEIGHT - 100);
            }
            xy.add(new enem (x2, y2, true)); 
            ranBluSpwn = 1;
        }
